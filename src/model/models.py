...
class ColNeRFNet(torch.nn.Module):
    def __init__(self, conf, nviews, data_format=None, stop_encoder_grad=False):
        ...
        block_size = 9
        if data_format == "dvr_dtu":
            block_size = 5
        self.interview_fusion = InterviewFusion(nviews=nviews, block_size=block_size, checkpoint=conf.get_bool("use_checkpoint", False))
        self.DownSamp = torch.nn.AvgPool2d(2, 2)
        self.if_patch = False
    ...
    def encode(self, images, poses, focal, z_bounds=None, c=None):
        ...
        if len(images.shape) == 5:
            ...
            poses = poses[..., :3, :4].reshape(-1, 3, 4)
        else:
            self.num_views_per_obj = 1

        initial_latent = self.encoder(images)
        interview_latent = self.interview_fusion(initial_latent)
        interview_latent = torch.stack(interview_latent, dim=1).reshape(-1, initial_latent.shape[-3], initial_latent.shape[-2], initial_latent.shape[-1])
        rgb_embed = self.DownSamp(images * 0.5 + 0.5).to(interview_latent.device)
        self.encoder.latent = torch.cat((interview_latent, rgb_embed), dim=1)
    ...
    def forward(self, xyz, coarse=True, viewdirs=None, far=False):
        ...
        with profiler.record_function("model_inference"):
            ...
            if self.d_in > 0:
                ...
                if self.use_xyz:
                    if self.normalize_z:
                        ...
                        # z_feature = xyz_w.reshape(-1, 3)  # (SB*B, 3)
                    else:
            ...
            if self.use_encoder:
                ...
                latent, rgb_mask = self.encoder.index(
                    uv, None, self.image_shape
                )  # (SB * NS, latent+3, B), # (SB*NS, B)
                latent_size = latent.shape[1]-3
                rgb_pseudo = latent[:, -3:, :]# (SB * NS, 3, B)
                rgb_pseudo = rgb_pseudo.reshape(SB, NS, 3, -1)
                rgb_mask = rgb_mask.reshape(SB, NS, 1, -1)
                mask_rgb_pseudo = rgb_pseudo * rgb_mask
                mask_rgb_pseudo = mask_rgb_pseudo.sum(dim=1) / (rgb_mask.sum(dim=1) + 1e-8)
                mask_rgb_pseudo = mask_rgb_pseudo.permute(0, 2, 1)
                newmask = torch.any(rgb_mask != 0, dim=1).float().permute(0, 2, 1) # SB, B, 1
                mask_rgb_pseudo = torch.cat((mask_rgb_pseudo, newmask), dim=-1)
            ...
            if coarse or self.mlp_fine is None:
                # if isnan(torch.any(mlp_input)):
                #     print("mlp_input has NAN!!!!!!!!!")
            ...
            output = torch.cat((output, mask_rgb_pseudo), dim=-1) # B, K, 7(3 1 4)
        return output
    ...
    def load_weights(self, args, opt_init=False, strict=True, device=None):
        ...
        Your can put a checkpoint at checkpoints/<exp>/colnerf_init to use as initialization.
        ...
        if opt_init and not args.resume:
            return
        ckpt_name = (
            "colnerf_init" if opt_init or not args.resume else "colnerf_latest"
        )
        nv = args.nviews + 'v'
        model_path = "%s/%s/%s/%s" % (args.checkpoints_path, args.name, nv, ckpt_name)
    ...
    def save_weights(self, args, opt_init=False):
        ...
        ckpt_name = "colnerf_init" if opt_init else "colnerf_latest"
        backup_name = "colnerf_init_backup" if opt_init else "colnerf_backup"
    ...
    def save_best_weights(self, args, opt_init=False):
        """
        Helper for saving weights according to argparse arguments
        :param opt_init if true, saves from init checkpoint instead of usual
        """
