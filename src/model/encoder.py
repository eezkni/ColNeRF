        ...
        with profiler.record_function("encoder_index"):
            ...
            with profiler.record_function("encoder_index_pre"):
                if len(image_size) > 0:
                    ...
                    uv = uv * scale - 1.0
                    mask11 = (uv[..., 0].reshape(-1, 1) > -1).float()
                    mask12 = (uv[..., 0].reshape(-1, 1) < 1).float()
                    mask21 = (uv[..., 1].reshape(-1, 1) > -1).float()
                    mask22 = (uv[..., 1].reshape(-1, 1) < 1).float()
                    mask = mask11 * mask12 * mask21 * mask22
                    mask = mask.reshape(uv.shape[0], uv.shape[1])
            ...
            # if isnan(torch.any(samples)):
            #     print("Samples has nan!!!!!!")
