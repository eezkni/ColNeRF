...
class ResnetFC(nn.Module):
    def __init__(
        ...
        if d_in > 0:
            ...
            # self.lin_d = nn.Linear(3, d_hidden)
            # nn.init.constant_(self.lin_d.bias, 0.0)
            # nn.init.kaiming_normal_(self.lin_d.weight, a=0, mode="fan_in")
        ...
        self.out_sigma_layer = out_sigma_layer
    ...
    def forward(self, zx, if_patch, combine_inner_dims=(1,), combine_index=None, dim_size=None):
        """
        ...
        with profiler.record_function("resnetfc_infer"):
            ...
            if self.d_latent > 0:
                ...
                # if if_patch:
                #     x = x * 0.2
                # x = zx[..., self.d_latent : self.d_latent+self.d_in-3]
                # d = zx[..., self.d_latent+self.d_in-3:]
                # d = d.reshape(-1, *combine_inner_dims, d.shape[-1])[:, 0, ...]
                # d = d.reshape(-1, 3)
            else:
                x = zx
            if self.d_in > 0:
            ...
            for blkid in range(self.n_blocks):
                ...
                if self.use_checkpoint:
                    x = checkpoint(self.blocks[blkid],x)
                else:
                    x = self.blocks[blkid](x)
            out = self.lin_out(self.activation(x))
            # rgb_out = self.lin_out(self.activation(x))
    ...
    def from_conf(cls, conf, d_in, **kwargs):
        ...
        return cls(
            ...
            use_checkpoint = conf.get_bool("use_checkpoint", False),
