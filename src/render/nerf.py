...
class _RenderWrapper(torch.nn.Module):
    ...
    def forward(self, rays, want_weights=False):
        ...
        outputs, coarse_results, fine_results  = self.renderer(
        ...
        if self.simple_output:
            ...
            return outputs.toDict(), coarse_results, fine_results
...
class NeRFRenderer(torch.nn.Module):
    ...
    def composite(self, model, rays, z_samp, coarse=True, sb=0):
        ...
        with profiler.record_function("renderer_composite"):
            ...
            out = out.reshape(B, K, -1)  # (B, K, 8)

            rgbs = out[..., :3]  # (B, K, 3)
            sigmas = out[..., 3]  # (B, K)
            rgb_pseudos = out[..., -4:]  # (B, K, 3)
            if self.training and self.noise_std > 0.0:
                sigmas = sigmas + torch.randn_like(sigmas) * self.noise_std

            raw2alpha = lambda raw, dists, act_fn=F.relu: 1.-torch.exp(-act_fn(raw)*dists)
            
            alphas = raw2alpha(sigmas, deltas)  # [N_rays, N_samples]
            ...
            return (
                ...
                rgb_final,
                depth_final,
                torch.relu(rgb_pseudos),
                torch.relu(rgbs),
                torch.relu(alphas_out),
                torch.relu(acc),
            )
    ...
    def forward(
        ...
        with profiler.record_function("renderer_forward"):
            ...
            bs = outputs.coarse.rgb.shape[0]
            coarse = [x.reshape(bs, -1, x.shape[-1]) for x in coarse_composite[3:6]] # rgb_psudo:bs, npoints, 4; rgb:bs, npoints, 3; density:bs, nrays, npoints;
            coarse.append(coarse_composite[-1].reshape(bs, -1, 1)) # acc:, bs, nrays, 1
            fine = [x.reshape(bs, -1, x.shape[-1]) for x in fine_composite[3:6]] 
            fine.append(fine_composite[-1].reshape(bs, -1, 1))
            return outputs, coarse, fine

    def _format_outputs(
        self, rendered_outputs, superbatch_size, want_weights=False,
    ):
        weights, rgb, depth, rgb_pseudos, rgbs, density, acc= rendered_outputs
