...
if args.dataset_format == "dvr":
    ...
    list_name = ""
    img_dir_name = "rgb"
elif args.dataset_format == "llff":
    list_name = ""
    img_dir_name = "images_8"
else:
...
def run_map():
    ...
    def get_metrics(rgb, gt):
        ssim = structural_similarity(rgb, gt, multichannel=True, data_range=1)
        psnr = peak_signal_noise_ratio(rgb, gt, data_range=1)
        return psnr, ssim
    ...
    def process_obj(path, rend_path):
        ...
        for i, im_name in enumerate(ims):
            ...
            if args.dataset_format == "llff":
                if "DJI" in im_name:
                    im_name_id = i
                else:
                    im_name_id = int(osp.splitext(im_name)[0].replace("image", ""))
            else:
...
def run_reduce():
    ...
    all_objs = []
    objs = [x for x in os.listdir(render_root) if x != "finish.txt"]
