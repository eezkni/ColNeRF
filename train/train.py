...
def extra_args(parser):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=None,
        help="if add ray RGB pseudo loss",
    )
    parser.add_argument(
        "--density_reg",
        action="store_true",
        default=None,
        help="if add ray geometry regularization loss",
    )
...
net = make_model(conf["model"], data_format=args.dataset_format, nviews=int(args.nviews[0])).to(device)
...
class ColNeRFTrainer(trainlib.Trainer):
    def __init__(self):
        ...
        self.color_reg = args.color_reg
        self.density_reg = args.density_reg
        self.Pesudo_smooth_loss = loss.SmoothingLoss()
        self.Pesudo_RGBloss = loss.RGBSmooth()
        self.lambda_smooth = conf.get_float("loss.lambda_smooth", 0.00002)
        self.lambda_rgb_smooth = conf.get_float("loss.lambda_rgb_smooth", 0.00002)
        self.depthweight = 0.01
    ...
    def calc_losses(self, data, is_train=True, global_step=0, ):
        ...
        for obj_idx in range(SB):
            ...
            if all_bboxes is not None:
                ...
                if self.density_reg:
                    pix_inds = util.pseudo_sample(
                        H, W, NV, args.ray_batch_size
                    )  # (NV, H, W, 8)(0, NV * H * W, (args.ray_batch_size,))\
                else:    
        ...
        render_results = render_par(all_rays, want_weights=True,)
        render_dict = DotMap(render_results[0])
        
        coarse_add = render_results[1]
        fine_add = render_results[2]
        ...
        loss = 0.0
        if self.density_reg:
            num_ray = args.ray_batch_size//8
            depth_1 = coarse.depth[:, 6*num_ray:7*num_ray]
            depth_2 = coarse.depth[:, 7*num_ray:]
            Ray_loss = self.Pesudo_smooth_loss(coarse_add[2], coarse_add[3], depth_1, depth_2, weight=self.depthweight) * self.lambda_smooth
            loss_dict["reg_dc"] = Ray_loss.item()
            loss = Ray_loss
            if using_fine:
                depth_1 = fine.depth[:, 6*num_ray:7*num_ray]
                depth_2 = fine.depth[:, 7*num_ray:]
                Fine_Ray_loss = self.Pesudo_smooth_loss(fine_add[2], fine_add[3], depth_1, depth_2, weight=self.depthweight) * self.lambda_smooth
                loss_dict["reg_df"] = Fine_Ray_loss.item()
                loss = loss+Fine_Ray_loss
                
        if self.color_reg:
            Ray_rgb_loss = self.Pesudo_RGBloss(coarse_add[0], coarse_add[1]) * self.lambda_rgb_smooth
            loss_dict["reg_cc"] = Ray_rgb_loss.item()
            loss = loss + Ray_rgb_loss
            if using_fine:
                FRay_rgb_loss = self.Pesudo_RGBloss(fine_add[0], fine_add[1]) * self.lambda_rgb_smooth
                loss_dict["reg_cf"] =FRay_rgb_loss.item()
                loss = loss + FRay_rgb_loss
                
        rgb_loss = self.rgb_coarse_crit(c_rgb, all_rgb_gt)
        loss_dict["rc"] = rgb_loss.item() * self.lambda_coarse
        if using_fine:
            fine_loss = self.rgb_fine_crit(f_rgb, all_rgb_gt)
    ...
    def vis_step(self, data, global_step, idx=None):
        ...
        with torch.no_grad():
            ...
            render_results = render_par(test_rays, want_weights=True,)
            render_dict = DotMap(render_results[0])
            coarse = render_dict.coarse
            fine = render_dict.fine
